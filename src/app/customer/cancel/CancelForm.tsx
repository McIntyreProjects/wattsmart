...
export function CancelForm({
  ...
  const refundPence = withinCoolingOff ? amountPence : amountPence - feePence
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      // The server computes the authoritative amounts — use them for the confirmation
      const refunded = typeof json.refundedAmount === 'number' ? json.refundedAmount : refundPence
      const fee = typeof json.feeRetained === 'number' ? json.feeRetained : (withinCoolingOff ? 0 : feePence)
      const params = new URLSearchParams({
        refund: (refunded / 100).toFixed(2),
        deposit: (amountPence / 100).toFixed(2),
          ...
          <div>
            ...
            <div className="border border-ws-border rounded-tile p-4 mb-5 bg-white">
              <div className="flex justify-between text-sm mb-2">
                ...
                <span className="font-semibold">{formatCurrency(amountPence)}</span>
              </div>
              <div className="border-t border-ws-border pt-2 flex justify-between text-sm">
                <span className="text-ws-muted">Your refund</span>
                <span className="font-bold text-ws-green">{formatCurrency(refundPence)}</span>
              </div>
            ...
            >
              {loading ? 'Processing…' : `Yes, cancel & refund ${formatCurrency(refundPence)}`}
            </button>
          ...
          <div>
            ...
            <div className="border border-ws-border rounded-tile p-4 mb-4 bg-white text-sm">
              ...
              <div className="flex justify-between mb-2">
                ...
                <span className="text-ws-red-text">– {formatCurrency(feePence)}</span>
              </div>
              <div className="flex justify-between mb-2">
              ...
              <div className="border-t border-ws-border pt-2 flex justify-between text-xs text-ws-muted">
                ...
                <span className="font-bold text-ws-red-text">{formatCurrency(feePence)}</span>
              </div>
            </div>
